...
├── backtrader/                 # 📈 Backtrader 回测与实盘框架 (新增)
...
## 📈 Backtrader 回测与实盘 (Backtrader Integration)

`binance_bot` 现已集成 Backtrader 框架，支持更专业的回测与实盘交易。

### 功能亮点
*   **统一框架**: 历史回测与实盘交易使用同一套策略代码。
*   **CCXT 集成**: 通过 CCXT 无缝对接币安合约接口。
*   **多策略支持**: 完美移植了 Martingale, Amplitude, Probability, Monitor 四大策略。

### 快速启动

**1. 运行马丁策略历史回测**
```bash
python3 binance_bot/backtrader/run_bt_test.py --strategy martingale --symbol BTC/USDT --days 30
```

**2. 运行纯历史回测 (无实时数据等待)**
```bash
python3 binance_bot/backtrader/run_bt_test.py --strategy martingale --no-realtime
```

**3. 实盘模式 (Live Trading)**
> ⚠️ 注意：实盘模式会使用您的真实资金开单，请务必在模拟模式充分测试。

```bash
python3 binance_bot/backtrader/run_bt_test.py --strategy martingale --mode live
```

## 🐋 Freqtrade 集成 (Freqtrade Integration)

`binance_bot` 也支持 [Freqtrade](https://www.freqtrade.io/) 框架，提供了更完善的实盘管理、UI 面板和 Telegram 控制。
...
│   ├── config.json              # 核心配置
│   └── strategies/
│       ├── MartingaleStrategy.py   # DCA/网格策略
│       └── ProbabilityStrategy.py  # 均值回归策略
```
...
```bash
cd binance_bot/freqtrade
docker-compose up -d
```
*   这会启动 Freqtrade 并加载 `MartingaleStrategy`。
*   WebUI 访问地址: `http://localhost:8080` (默认用户/密码见 `config.json`)。

更多详情请参考 [freqtrade/README.md](binance_bot/freqtrade/README.md)。
